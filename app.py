...
def ai_full_setup():
    ...
    # Guard against None from DB — default to 'Neutral' / 'Middle'
...
Recent 30-day spending: {dict(recent_spending)}
Selected categories: {', '.join(selected_categories)}
Rules:
- Saver mindset: prioritise needs & savings.
- Spender mindset: allow more wants.
- Values MUST sum to exactly 100.
Return ONLY valid JSON, no markdown. Keys = category names, values = float percentages.
Example: {{"Food & Dining": 35.0, "Transport": 15.0, "Savings": 20.0}}"""
 
        raw = route_ai_request(prompt, max_tokens=600)
 
        # ── Robust parse (guard against None and markdown fences) ──────
        if raw:
            try:
                cleaned = raw.strip()
                # Strip ```json ... ``` fences
                ...
                if isinstance(parsed, dict):
                    # Keep only selected categories; ensure float values
                    allocation = {
                        k: float(v)
                    ...
                    # ── Detect decimal fractions (0–1) and scale to % ─
                    if allocation and max(allocation.values()) <= 1.5:
                        allocation = {k: round(v * 100, 2) for k, v in allocation.items()}
 
            except Exception as parse_err:
                print(f"AI parse error: {parse_err} — using fallback")
                allocation = {}
 
    # ── 2. Local ML fallback if AI returned nothing usable ──────────────
    if not allocation:
        ...
        other_cats   = [c for c in selected_categories
    ...
    if OPENROUTER_API_KEY or GEMINI_API_KEY:
        ...
        sp_prompt = (
            f"User income \u20b1{monthly_income:.2f}, expenses \u20b1{sum(recent_spending.values()):.2f}, "
        ...
        if sp_raw:
            try:
                sp_clean = sp_raw.strip().strip('`').lstrip('json').strip()
    ...
    if OPENROUTER_API_KEY or GEMINI_API_KEY:
        ...
        if adv_raw:
            try:
                adv_clean = adv_raw.strip().strip('`').lstrip('json').strip()
    ...
    # ── 7. Persist allocations + budgets to DB ───────────────────────────
    needs_set_local  = {'Food & Dining', 'Transport', 'Groceries',
                        'Health', 'Debt repayment', 'Mortgage'}
    savings_set_local = {'Savings'}
 
    UserAllocation.query.filter_by(user_id=user.id).delete()
    for cat, pct in allocation.items():
        cat_type = ('need'    if cat in needs_set_local  else
                    'savings' if cat in savings_set_local else 'want')
        db.session.add(UserAllocation(
            user_id=user.id, category_name=cat,
            type=cat_type, percentage=pct))
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=port, debug=False)import json
import csv
import io
import os
import requests
