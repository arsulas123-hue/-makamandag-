...
class Transaction(db.Model):
    ...
    is_need = db.Column(db.Boolean, default=True)      # meaningful for expense
    priority = db.Column(db.Integer, default=1)        # 0-3 (Low to Critical)
...
def compute_health_score(user_id):
    """Calculate a health score from 0-100 based on spending patterns."""
    ...
    # Want vs Need ratio for expenses
    ...
    # Base score 70, adjust
    score = 70
    score += int(savings_rate * 20)          # up to +20
    score -= int(want_ratio * 15)            # up to -15
    score -= min(20, int(overspend_penalty * 10))  # up to -20

    return max(0, min(100, score))


def generate_weekly_forecast(user_id):
    """Return a simple 4-week spending forecast using linear regression on daily spend."""
    thirty_days_ago = datetime.utcnow() - timedelta(days=30)
    expenses = Transaction.query.filter(
        Transaction.user_id == user_id,
        Transaction.tx_type == 'expense',
    ...
    if len(expenses) < 7:
        ...
        # Group by day and compute trend
        ...
        if n > 1:
            ...
            # Predict daily spend for next 28 days, then sum into weeks
            week_sums = [0, 0, 0, 0]
    ...
    # Fallback: average daily * 7 per week
...
def generate_advice(user_id):
    """Generate smart AI advice based on spending patterns."""
    user = User.query.get(user_id)
    if not user:
        return []
...
def get_monthly_summary(user_id):
    """Return balance, total expense/income for current month, and monthly history."""
    user = User.query.get(user_id)
    if not user:
...
def compute_longevity(user_id):
    """Estimate how many days the current balance will last based on average daily spend."""
    user = User.query.get(user_id)
    if not user:
        return {'balance': 0, 'avg_daily_spend': 0, 'days': 0}
...
def chat():
    ...
    if 'health' in message or 'score' in message:
        score = compute_health_score(user_id)
        return jsonify({'reply': f"Your financial health score is {score}/100. This is based on savings rate, want-to-need ratio, and budget adherence."})
    ...
    if 'investment' in message:
        return jsonify({'reply': "Based on your spending habits, consider putting 20% of your monthly surplus into low-cost index funds or a high-yield savings account."})
    ...
    if 'needs vs wants' in message:
        ...
        total = wants + needs
        if total > 0:
            reply = f"Your spending: Needs = ₱{needs:,.2f} ({needs/total*100:.0f}%), Wants = ₱{wants:,.2f} ({wants/total*100:.0f}%). Try to keep wants below 30%."
        else:
            reply = "No expense data yet. Add some transactions to see needs vs wants breakdown."
        return jsonify({'reply': reply})

    if 'priority' in message:
        return jsonify({'reply': "Use priority levels (0=Low,3=Critical) when adding expenses. This helps ML adjust budget recommendations."})

    # default fallback
    return jsonify({'reply': "I can help with: 'health score', 'forecast', 'investment', 'needs vs wants', 'priority'. Try one of those!"})
