...
def compute_health_score(user_id):
    ...
    # FIX: skip categories with limit == 0 to avoid division by zero
...
# ----------------------------------------------------------------------
# Apply future expenses (ENHANCED to handle recurring)
# ----------------------------------------------------------------------
...
<body>
<!-- ... (entire HTML unchanged up to the dashboard card after AI plan) ... -->

<!-- Budget Management card (added after AI allocation card) -->
...
// Add call to loadBudgets() after AI plan is rendered
const originalRenderAIPlan = renderAIPlan;
renderAIPlan = function(plan) {
    originalRenderAIPlan(plan);
    loadBudgets();
};
