...
class User(db.Model):
    ...
    monthly_budget_limit = db.Column(db.Float, default=0.0)   # user-defined monthly cap
...
class FutureExpense(db.Model):
    ...
    amount = db.Column(db.Float, nullable=False)
    category = db.Column(db.String(50), nullable=False)
    cycle = db.Column(db.String(20), default='One-time')   # One-time, Weekly, Monthly
...
class UserAllocation(db.Model):
    ...
    type = db.Column(db.String(10), nullable=False)   # 'need' or 'want'
...
@app.route('/')
def index():
    # Serve the HTML UI (you must have index.html in same folder)
...
# User profile update (social status, mindset, monthly budget cap)
...
# ----------------------------------------------------------------------
# NEW ENDPOINTS for Future Expenses and Allocations
# ----------------------------------------------------------------------
...
# User category allocations (Need/Want + percentage)
...
def update_allocations():
    user = get_current_user()
    data = request.json  # list of {category_name, type, percentage}
