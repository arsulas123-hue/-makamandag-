...
# Configure Gemini AI (uses environment variable GEMINI_API_KEY)
...
@login_required
def ai_analyze():
    """Separate endpoint for AI analysis (used by chat fallback)."""
    user = get_current_user()
    data = request.json
...
def chat():
    ...
    # 1. Keyword-based commands (fast, no API call)
    ...
    # 2. For everything else, use Gemini AI
    try:
        # Reuse the ai_analyze logic (but we call it directly to avoid duplicate code)
        user_data = {
            "balance": get_monthly_summary(user.id)["balance"],
            "recent_transactions": [
                t.to_dict() for t in Transaction.query
...
# ----------------------------------------------------------------------
# Schema migration – adds missing columns AND removes obsolete 'password' column
# ----------------------------------------------------------------------
def ensure_schema():
    inspector = inspect(db.engine)
    if inspector.has_table('users'):
