...
# Add all other routes (logout, me, transactions, summary, predict, budgets, admin) exactly as in the previous working version.
# For brevity, I've included only the critical fixes here. The rest of the routes remain the same as the last working version.
# You can copy them from the previous clean app.py I provided.

# -------------------- Catch-all for frontend --------------------
@app.route('/', defaults={'path': ''})
@app.route('/<path:path>')
def serve_index(path):
    # If the path starts with "api/", it's an unmatched API route -> return JSON 404
    if path.startswith('api/'):
        return jsonify({'error': 'API endpoint not found'}), 404
    # Serve static files if they exist, else index.html (for client-side routing)
